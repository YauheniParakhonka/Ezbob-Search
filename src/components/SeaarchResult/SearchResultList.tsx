...
export const SearchResults: React.FC<SearchResultsProps> = ({searchValue}) => {
    ...
    const [itemsPerPage, setLimit] = useState<number>(10);
    ...
    useEffect(() => {
        setIsLoading(true);
        getResponseWithPagination({searchQuery: searchValue, page: currentPage, itemsPerPage}).then((response) => {
        ...
        <div className={styles.container}>
            ...
            <ul className={styles.list}>
                {values.map(({id, title, description}) => (
                    <SearchResultItem key={id} id={id} title={title} description={description}/>
                ))}
            </ul>
            <div className={styles.pagination}>
                {Array(totalPages).fill(0).map((_, i) => {
                    const page = i + 1;
                    return <div key={page} onClick={() => setPage(page)}
                                className={clsx(styles.pagination_item, {[styles.pagination_item_active]: page === currentPage})}>{page}</div>
                })}
            </div>
        </div>
    );
